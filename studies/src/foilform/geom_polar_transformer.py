"""Geometry sequence → Cl, Cd predictor (studies: swappable block types).

Block types: pairwise (default), standard_mlp, no_pairwise (alias standard_mlp),
no_mlp (pairwise without middle MLP), attention_only.
"""
...
# Registry keys used by CLI / run_all
BLOCK_TYPES = (
    "pairwise",
    "full",
    "standard_mlp",
...
class AttentionPairwiseBlock(nn.Module):
    """Attention + pairwise + MLP + pairwise (full block)."""
...
class StandardAttentionMLPBlock(nn.Module):
    """Causal self-attention + MLP (no pairwise), same QKV style as pairwise block."""
    ...
    def __init__(self, d: int = D_MODEL, dropout: float = DEFAULT_DROPOUT) -> None:
        ...
        self.mlp_w2 = nn.Parameter(torch.empty(d, 16))
        self.mlp_b2 = nn.Parameter(torch.empty(d))
        for p in (self.W_q, self.W_k, self.W_v, self.mlp_w1, self.mlp_w2):
            nn.init.xavier_uniform_(p)
        nn.init.zeros_(self.mlp_b1)
        nn.init.zeros_(self.mlp_b2)
...
class AttentionOnlyBlock(nn.Module):
    """Causal self-attention residual only."""
...
class AttentionPairwiseNoMLPBlock(nn.Module):
    """Attention + pairwise + pairwise; middle MLP branch removed."""
...
def make_block(block_type: str, d: int, dropout: float) -> nn.Module:
    t = block_type.lower().strip()
    if t in ("pairwise", "full"):
        return AttentionPairwiseBlock(d, dropout=dropout)
    if t in ("standard_mlp", "no_pairwise"):
        return StandardAttentionMLPBlock(d, dropout=dropout)
    if t == "no_mlp":
        return AttentionPairwiseNoMLPBlock(d, dropout=dropout)
    if t == "attention_only":
        return AttentionOnlyBlock(d, dropout=dropout)
    raise ValueError(f"Unknown block_type {block_type!r}; expected one of {BLOCK_TYPES}")
    ...
    *,
    ...
    if aero.dim() == 2:
        if aero.shape[0] != batch or aero.shape[1] != d_model:
            raise ValueError(f"aero (B,d) expected ({batch}, {d_model}), got {tuple(aero.shape)}")
        ...
        if aero.shape[0] != batch or aero.shape[1] != seq_len or aero.shape[2] != d_model:
            raise ValueError(
                f"aero (B,L,d) expected ({batch}, {seq_len}, {d_model}), got {tuple(aero.shape)}"
            )
        seq = aero
    else:
        raise ValueError(f"aero must be (B,d) or (B,L,d), got dim {aero.dim()}")
...
class GeomPolarTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.blocks = nn.ModuleList(
            [make_block(block_type, d_model, dropout) for _ in range(n_layers)]
        )
    ...
    def _forward_with_aero_embed(
        ...
        b, l, d = x.shape
        if d != self.d_model:
            raise ValueError(f"geom expected (B, L, {self.d_model}), got {tuple(x.shape)}")
    ...
    def forward_autoregressive(
        ...
        dm = self.d_model
        if d != dm:
            raise ValueError(f"geom expected last dim {dm}, got {d}")
    ...
    def decode_append(
        ...
        *,
        ...
        dm = self.d_model
        if d != dm:
            raise ValueError(f"geom_context expected last dim {dm}, got {d}")
        ...
        for t in range(1, target_steps):
            ...
            aero_s_new = torch.zeros(b, 1, d, device=geom_context.device, dtype=geom_context.dtype)
            x_new = self.drop_in(
