#!/usr/bin/env python3
"""Train PolarCorrectionMLP on top of transformer predictions (learns Δ)."""
...
def build_geom_xy(coords: np.ndarray) -> np.ndarray:
    """(N, 501, 2) → (N, 501, 2) x and y per station."""
...
def build_polar34_and_mask(polars: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    """Pack polars (N, K, 3) into (N, 34) = [Cl×17, Cd×17] + mask."""
    n, k, _ = polars.shape
    if k > N_SLOTS:
...
def build_targets_seq(polars: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    """Same packed (N, S, 3) targets as the transformer trainer."""
...
def column_indices_for_airfoils(polars: np.ndarray) -> list[list[int]]:
    """For each airfoil, which polar columns j are valid."""
    out = []
...
def generate_transformer_base(
    ...
    """Run the transformer on all airfoils and pack predictions into (N, 34) layout."""
    state = torch.load(ckpt_path, map_location=device, weights_only=False)
    n_l = infer_transformer_n_layers(state["model"])
    model = GeomPolarTransformer(n_layers=n_l).to(device)
...
def main() -> None:
    ...
    for p in (coords_path, polars_path, geom_emb_path):
        if not p.is_file():
            raise FileNotFoundError(f"Missing {p} — run prepare_data / train_tokenizers first.")
    ...
    base34 = generate_transformer_base(
        geom_emb, y_seq, m_seq, polars, col_indices, geom_ckpt, device,
    )
    ...
    with (run_dir / "args.json").open("w", encoding="utf-8") as f:
        json.dump(
            {**vars(args), "total_params": total_n, "geom_checkpoint": str(geom_ckpt)},
            f, indent=2,
        )
    ...
    for ep in range(1, args.epochs + 1):
        ...
        for geom_b, tgt_b, mask_b, base_b in train_loader:
            geom_b, tgt_b, mask_b, base_b = (
                geom_b.to(device), tgt_b.to(device), mask_b.to(device), base_b.to(device),
            )
        ...
        row = {
            "epoch": ep, "train_mse": tr_mse, "val_mse": va_mse,
            "val_mae_cl": va_mae_cl, "val_mae_cd": va_mae_cd,
        }
        history.append(row)
        print(
        ...
        if np.isfinite(va_mse) and va_mse < best_val:
            best_val = va_mse
            torch.save(
                {"model": model.state_dict(), "epoch": ep, "best_val_mse": best_val, "args": vars(args)},
                ckpt,
            )
