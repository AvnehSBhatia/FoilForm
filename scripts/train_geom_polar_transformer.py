#!/usr/bin/env python3
"""Train GeomPolarTransformer with 60/40 airfoil split (balanced MSE + raw MAE logs).

Supervised outputs are Cl and Cd only. AoA is never predicted: each decode step appends
``tuple_to_embed([pred_Cl, pred_Cd, ground_truth_AoA])`` so the model learns polar curves
conditioned on the known angle schedule (same as ``decode_append(..., aoa_ground_truth=...)``).
"""
...
def build_targets(polars: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    """Pack valid tuples per airfoil into left-aligned (N, S, 3) with mask (N, S)."""
    ...
    for i in tqdm(range(n), desc="build_targets", unit="airfoil"):
        ...
        for j in range(n_cols):
            # keep same validity rule as rest of project: Cl finite => valid sample
...
def masked_mae(pred: torch.Tensor, target: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
    # pred: (B,S,2), target: (B,S,3) uses [:,:,:2], mask: (B,S)
    w = mask.unsqueeze(-1)
    num = torch.abs(pred - target) * w
...
def compute_channel_scales(y: np.ndarray, mask: np.ndarray) -> np.ndarray:
    """Train-set scale per channel for balanced MAE (Cl, Cd)."""
    y2 = y[..., :2]
...
def masked_weighted_mse(
    ...
    """
    Balanced MSE: (e_c / scale_c)^2, masked over variable sequence lengths.
    channel_scales: (2,) in target order [Cl, Cd]. target: (B,S,3) uses first two channels.
    """
    w = mask.unsqueeze(-1)
    err = pred - target[..., :2]
...
def masked_channel_mae(
    ...
    """Returns raw per-channel MAE tensor of shape (2,) for [Cl, Cd]."""
    w = mask.unsqueeze(-1)
...
def autocast_cm(device: torch.device):
    # MPS float16 can destabilize this deep autoregressive stack; keep MPS fp32.
    if device.type == "cuda":
...
def main() -> None:
    ...
    parser.add_argument("--patience", type=int, default=20, help="Early-stop patience in validation checks.")
    parser.add_argument("--min_delta", type=float, default=1e-5, help="Minimum val improvement to reset patience.")
    parser.add_argument("--early_stop_warmup", type=int, default=10, help="Do not early-stop before this epoch.")
    ...
    if not geom_path.is_file():
        raise FileNotFoundError("geom_embeddings.npy not found. Run scripts/train_tokenizers.py first.")
    if not polars_path.is_file():
        raise FileNotFoundError("polars.npy not found.")

    geom = np.load(geom_path).astype(np.float32)  # (N,167,8)
    polars = np.load(polars_path).astype(np.float32)  # (N,9,3)
    ...
    y, m = build_targets(polars)  # (N,S,3), (N,S)
    n_steps = y.shape[1]
    ...
    print(
        ...
        f"(train_frac={args.train_frac:.2f}); target_steps={n_steps}",
    )
